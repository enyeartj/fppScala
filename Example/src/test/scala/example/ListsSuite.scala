 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of empty list returns zero`: Unit = {
    Assert.assertEquals(0, sum(List()))
  }

  @Test def `one plus negative five equals negative four`: Unit = {
    Assert.assertEquals(-4, sum(List(1,-5)))
  }
  ...
  @Test def `max of negative numbers`: Unit = {
    Assert.assertEquals(-4, max(List(-7,-10,-4,-5)))
  }
